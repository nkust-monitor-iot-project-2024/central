...
// The InvaderFunc type is an adapter to allow the use of ordinary
// function as Invader mutator.
type InvaderFunc func(context.Context, *ent.InvaderMutation) (ent.Value, error)

// Mutate calls f(ctx, m).
func (f InvaderFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	if mv, ok := m.(*ent.InvaderMutation); ok {
		return f(ctx, mv)
	}
...
// Mutate calls f(ctx, m).
func (f MovementFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	if mv, ok := m.(*ent.MovementMutation); ok {
		return f(ctx, mv)
	}
	return nil, fmt.Errorf("unexpected mutation type %T. expect *ent.MovementMutation", m)
}

...
// QueryInvaders chains the current query on the "invaders" edge.
func (eq *EventQuery) QueryInvaders() *InvaderQuery {
	query := (&InvaderClient{config: eq.config}).Query()
...
func (eq *EventQuery) QueryMovements() *MovementQuery {
	query := (&MovementClient{config: eq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := eq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
// WithInvaders tells the query-builder to eager-load the nodes that are connected to
// the "invaders" edge. The optional arguments are used to configure the query builder of the edge.
func (eq *EventQuery) WithInvaders(opts ...func(*InvaderQuery)) *EventQuery {
	query := (&InvaderClient{config: eq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	eq.withInvaders = query
	return eq
}
...
func (eq *EventQuery) WithMovements(opts ...func(*MovementQuery)) *EventQuery {
	query := (&MovementClient{config: eq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (eq *EventQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Event, error) {
	...
	if query := eq.withInvaders; query != nil {
		if err := eq.loadInvaders(ctx, query, nodes,
			func(n *Event) { n.Edges.Invaders = []*Invader{} },
			func(n *Event, e *Invader) { n.Edges.Invaders = append(n.Edges.Invaders, e) }); err != nil {
			return nil, err
		}
	}
	if query := eq.withMovements; query != nil {
...
func (eq *EventQuery) loadInvaders(ctx context.Context, query *InvaderQuery, nodes []*Event, init func(*Event), assign func(*Event, *Invader)) error {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(event.InvadersTable)
		s.Join(joinT).On(s.C(invader.FieldID), joinT.C(event.InvadersPrimaryKey[1]))
		s.Where(sql.InValues(joinT.C(event.InvadersPrimaryKey[0]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(event.InvadersPrimaryKey[0]))
	...
	neighbors, err := withInterceptors[[]*Invader](ctx, query, qr, query.inters)
	if err != nil {
		return err
	}
	for _, n := range neighbors {
		nodes, ok := nids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected "invaders" node returned %v`, n.ID)
		}
		for kn := range nodes {
			assign(kn, n)
		}
...
func (eq *EventQuery) loadMovements(ctx context.Context, query *MovementQuery, nodes []*Event, init func(*Event), assign func(*Event, *Movement)) error {
	edgeIDs := make([]driver.Value, len(nodes))
	byID := make(map[uuid.UUID]*Event)
	nids := make(map[uuid.UUID]map[*Event]struct{})
	for i, node := range nodes {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(event.MovementsTable)
		s.Join(joinT).On(s.C(movement.FieldID), joinT.C(event.MovementsPrimaryKey[1]))
		s.Where(sql.InValues(joinT.C(event.MovementsPrimaryKey[0]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(event.MovementsPrimaryKey[0]))
		s.AppendSelect(columns...)
		s.SetDistinct(false)
	})
	if err := query.prepareQuery(ctx); err != nil {
	...
	neighbors, err := withInterceptors[[]*Movement](ctx, query, qr, query.inters)
	if err != nil {
		return err
	}
	for _, n := range neighbors {
